...
export function DashboardPage() {
  ...
  const counts = useMemo(() => ({ normal: monitors.filter((m) => m.status === "normal").length, warning: monitors.filter((m) => m.status === "warning").length, error: monitors.filter((m) => ["connection_error", "read_error"].includes(m.status)).length }), [monitors]);
