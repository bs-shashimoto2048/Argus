...
def test_monitor_source(monitor_id: int, source: VideoSourceInput, db: Session = Depends(get_db)):
    ...
    password = monitor_service.resolve_check_password(db, source, monitor)
    ...
    return SourceTestResponse(connected=result.connected, source_type=result.source_type, width=result.width, height=result.height, fps=result.fps, error_code=result.error_code, message=result.message, resolved_url_hint=result.resolved_url_sanitized)
